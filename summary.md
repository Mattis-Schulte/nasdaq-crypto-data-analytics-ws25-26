## Task 1 — Preprocessing Pipeline (NASDAQ + Crypto + FX)

### Goal

Create *reproducible*, *analysis-ready* datasets with a uniform schema:

- `ticker`, `date`, `open`, `close` (daily)
- same columns aggregated to weekly (weekly)
- metadata table for NASDAQ tickers

Outputs are written to:

- `data/processed/df_nasdaq_daily.csv`
- `data/processed/df_nasdaq_weekly.csv`
- `data/processed/df_crypto_daily.csv`
- `data/processed/df_crypto_weekly.csv`
- `data/processed/df_nasdaq_meta.csv`
- optional: `df_eurusd_daily.csv`, `df_eurusd_weekly.csv`

The key requirement was that downstream tasks (Task 2/3) are independent of the quirks of the raw data source.
...
### 1) Normalization
The pipeline immediately forces a standard shape:

- clean column names (`lower + strip`)
- parse dates consistently (tz-naive timestamps)
- enforce numeric `open/close`
- treat non-positive values as invalid (`<=0 → NaN`)
- drop duplicate `(ticker, date)` rows

This ensures every later step (splits, yfinance merge, weekly aggregation) is operating on predictable data.

---

### 2) Split adjustment
To ensure price histories are consistent over time, stock splits must be adjusted. As to avoid "double-adjusting", the pipeline checks if splits are already reflected in the raw feed:

- For each split event, compare the observed "jump ratio" around the split date
- Confirm if it matches the split factor within a given tolerance (e.g. `TOL = 5%`)
- Only then adjust all prior prices by dividing by the split factor
...
As to preserve the given dataset as the authoritative source, the pipeline uses a hybrid approach with the local CSVs being the base feed with yfinance as a supplement. More specifically, yfinance is used for:

- filling missing `open/close`
- replacing extreme one-day errors ("spikes") when enabled
- extending history beyond the local dataset (base cutoff at 2025-12-12)

---

### 4) Outliers detection with per-ticker log-returns
A $5 move means something different for a $2 stock versus a $2,000 stock; in addition, log-returns are more symmetric and stable for financial data. Thus, outlier detection is based on per-ticker log-returns: 

- compute `ret_open`, `ret_close` as differences in log price
- apply **Tukey fences** (IQR rule) with `IQR_K = 1.5`:
    - Inspired by standard boxplot outlier detection, see: https://medium.com/data-and-beyond/outlier-detection-in-r-tukey-method-or-why-you-need-box-and-whiskers-3c35d9ad8fb3
- mark a day as outlier if open OR close return is outside fences
- replace with yfinance if yfinance has data for that day

---

### 5) Weekly aggregations
Weekly aggregations are built for the downstream portfolio/DCA logic:

- **equities:** `W-FRI` (week ending Friday)
- **crypto:** `W-SUN` (week ending Sunday)

Weekly open/close are:
...
The pipeline prints a standard QA snapshot at key stages. Below are the most important results.
...
Key takeaway: **the outlier replacement rate is very high with the current settings.**

This is the most important finding of Task 1:  
...
Build a baseline supervised model:

- predict **close(t+1)** using information available at day **t**
- use only close-based features (simple, transparent)
- evaluate properly with time splits (no shuffling)
...
Dataset source:
- `df_nasdaq_daily.csv` (Task 1 output)
...
Time splits:
- **Train:** 2015–2023
- **Validation:** 2024 (feature selection)
...
This is the key learning outcome of Task 3:
